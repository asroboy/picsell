...
class SecurityUiTagLib {
    ...
    def formContainer = { attrs, body ->
        ...
        if (type == 'search') {
            ...
            title = ""
//			message(code: 'spring.security.ui.' + type + '.header')
        } else {
...
// TODO encodeAsHTML
    ...
    def menu = { attrs ->
        ...
        lines << """$indent\t\t<li><a href="${createLink('search', controller)}">${
            message(code: 'spring.security.ui.search')
        }</a></li>"""
        if (!searchOnly) {
            lines << """$indent\t\t<li><a href="${createLink('create', controller)}">${
                message(code: 'spring.security.ui.create')
            }</a></li>"""
        }
        lines << indent + '\t</ul>'
        lines << indent + '</li>'
    ...
    def securityInfoTable = { attrs, body ->
        ...
        out << """
			<table class='info'>
				<caption>${
            message(code: 'spring.security.ui.menu.securityInfo.' + type, args: captionArgs)
        }</caption>"""
